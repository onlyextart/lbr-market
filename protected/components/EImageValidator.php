    ...
    protected function validateAttribute($object, $attribute) {
        ...
        if ($this->types !== null) {
            ...
            if (!in_array(strtolower($file->getExtensionName()), $types)) {
                $message = $this->typesError !== null ? $this->typesError : Yii::t('yii', 'Файл "{file}" не может быть загружен. Разрешены только файлы с расширениями: {extensions}.');
        ...
        if (isset($this->width, $this->height) && $data !== false) {
            if ($data[0] != $this->width && $data[1] != $this->height) {
                $message = $this->dimensionError ? $this->dimensionError : Yii::t('yii', 'Размер изображения должен быть {width}x{height}');
