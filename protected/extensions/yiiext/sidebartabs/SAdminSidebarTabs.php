...
class SAdminSidebarTabs extends CWidget {
	...
	{
		$style = Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('ext.yiiext.sidebartabs.style'),
			false,
			-1,
			YII_DEBUG
		);
		...
		$cs->registerScriptFile($style.'/sidebartabs.scripts.js');
		$cs->registerCssFile($style.'/sidebartabs.css');
	}
}